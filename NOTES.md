...
Local staging retention (the fixed 7-day period, OPERATIONS.md section 4.5)
is a different thing: it governs the local copy of an object that a disc
already holds.
...
```
   ...
   |  <repo>/cache/      |<-- rebuild ------|  physical discs         |
...
**Why hardlinks are independent files in Phase 1.** Content dedup already
stores a hardlinked file's data once, keyed by content, so storing every
hardlinked path as its own tree entry costs no extra disc bytes. Grouping the
entries also needs a deterministic id: the device-and-inode rule fails on a
mount that omits them, and its path-set fallback needs a fixed, escaped
encoding of a walk path (FORMAT.md section 6.16). That encoding rule is not
needed for anything else in Phase 1, so defining it only to serve a fallback
hash is deferred with the grouping feature itself. `hardlink_group` and
`HARDLINK_MEMBER` stay reserved, zero and clear, so a Phase 2 writer can turn
grouping on with a `version_minor` bump and no change to Phase 1 bytes.

**Why extended attributes and ACLs are reserved in Phase 1.** Phase 1 stores
Unix permissions only: `mode`, `uid` and `gid`. `XATTR`, `ACL_ACCESS`,
`ACL_DEFAULT`, `ACL_NFS4`, `WIN_SD` and `WIN_ADS` each need a per-platform
value registry, such as RFC 8881's NFSv4 ACE flag and mask constants, or the
exact padding of a Windows alternate data stream item. Building that registry
before a platform consumes it is wasted design work and a likely source of a
later incompatible change. The type numbers stay reserved in the TLV registry,
a Phase 1 reader treats them as unknown TLVs, and a later phase defines each
one's item layout with a `version_minor` bump.
...
| Hardlink identity | Repository-local `hardlink_group` id, reserved until Phase 2 | Never an inode number. |
...
| 16 | A restore runs out of staging space | A disc's own share of the plan exceeds free disk | The restore would stall | None |
...
### 2.20 Deferred designs

A deferred design differs from a rejected one: it is specified and reserved
in the format, kept out of Phase 1 by an explicit decision, and open to a
later version when its trigger condition holds. FORMAT.md keeps each item's
registry id and field reserved so that adding it later needs no layout
change.

**Bundle container.** A container file that packs many small objects behind
one index, so a directory of small files costs few filesystem entries and
few seeks. Deferred because Phase 1 stores every object as its own file, and
a probe measurement did not show the cost high enough yet to justify the
extra structure: a loop-mounted UDF 2.01 image (`mkudffs`, udftools 2.3)
showed an in-ICB embedding threshold around 1,800 bytes, a File Entry cost of
about 2 KiB per file above that threshold regardless of payload size, and a
populate time of about 19 s for 100,000 small files under 256-way fanout
directories versus about 0.4 s for the same payload packed into 1,000 bundle
files; a single flat 100,000-entry directory showed quadratic populate cost
instead. Trigger to reconsider: a source whose small files waste more than a
stated share of a disc's capacity to File Entry overhead.

**Delta objects.** Storing an object as a byte diff against a related base
object instead of storing it whole. Deferred pending a design that fixes:
the base must sit in the same run as the delta; the delta chain depth is 1,
so a delta never references another delta; the diff algorithm carries an id
from a registry; and the diff output is byte-deterministic across writers.

**Tree slices.** Splitting one large directory's tree object into several
objects instead of one, so a small change does not rewrite the whole
directory listing. Deferred pending a design that fixes: the split point is
content-defined, over a hash of the entry name; the split threshold is
...
| OC045 | Staging retention, fixed | 7 days | OPERATIONS.md 4.5 | No | Round retention. `gc --force-after` is the escape. |
| OC057 | `commit`'s unstable-file retry count, fixed | 1 | OPERATIONS.md 7.6 | No | One retry chosen arbitrarily. |
...
exits with code 1 at once when it cannot get it.
...
|---|---|
| 0.5.4 | **Flags and config keys cut (GitHub issue #60, item C).** `pack` loses `--physical-capacity` and the capacity-too-small-versus-physical machinery built only for it: `--capacity` may still be set below a medium's own size, but `pack` no longer tracks or checks a separate physical capacity. `pack` loses `--ref`, `--snapshot` and `--no-fec`: it always packs what is staged and always carries every pending ref, and `fec.scheme` in the config is the one FEC switch (`--fec` still overrides it on for one run). `ls`, `log` and `status` lose `--json`; `ls` loses `--unstable-only`; `log` loses `--limit`. `verify --heal` requires `--out`; healing in place is gone, and `--heal` with no `--out` is a usage error, exit code 2. `restore`, `ls`, `log` and `recover` lose `--disc`: a single `DISC-ROOT` positional argument and `--discs-dir` already cover naming one disc or several. The config keys `commit.restat_after_read`, `commit.retry_unstable`, `sources.exclude` and `staging.retain_after_clean` are deleted; in-flight change detection always runs at one retry, excludes stay available through `--exclude` and `.noahsarkignore`, and staging retention is a fixed 7 days, with `gc --force-after` as the escape. A config file that still holds a deleted key gets the same refusal an unknown key gets. No on-disc byte changed. |
| 0.5.3 | **The local cache moves into the repository (GitHub issue #60, item B4).** The cache directory is always `<repo>/cache/`, found the same way `<repo>/staging/` is found. The config key `cache.dir` is deleted; a config file that still holds it gets the same refusal an unknown key gets. `XDG_CACHE_HOME` and `~/.cache/noahsark/` are gone; the cache no longer keys itself by the repository uuid, since the repository directory itself is now the key. Deleting the repository directory deletes its cache with it, which is what the "rebuild" e2e scenario and the "cache is only an accelerator" e2e check already did by removing the whole repository. No on-disc byte changed. |
...
| 0.4.23 | **The operator view: traps, output, command names and no mandatory flag (GitHub issue #36).** A walk-through with the real binary found five traps, and each one is closed. `--capacity` no longer takes a bare number: it once meant sectors and reads as bytes, a factor of 2048 apart, so a bare number is now a usage error that lists the presets and shows a size example; one parser serves `pack --capacity`, `pack --physical-capacity` and the new config key. `pack` already packed partially and only refused a capacity that held not one object, but its refusal quoted a sector total that read as the size of all the data; it now names the smallest staged object and its size, then the capacity to use. `image build` loses `--capacity` and reads the image length from the packed tree's own `DISC.bin`, so a capacity can no longer be retyped wrong; a forced-capacity disc therefore gets an image of the forced target, not of the physical capacity. `commit` warns about each FIFO, socket and device node in the source, capped at 20 lines with a count, and the exit code stays 0; `restore` used to report them years later. Every damaged staged object, of every kind, now gives one error text that names the object and the cure, in place of `format: buffer too short`. The output drops "run": `pack` prints `packed disc SEQ "LABEL": N objects, B bytes` and keeps its "next steps" block, which the guide now repeats word for word; `verify` prints `disc SEQ "LABEL": N objects, ok` and loses `capacity_is_forced`, the run header copies, the stream blocks and the stripes; `plan` sorts by disc number then uuid and hides the pass split and the peak bytes unless a budget is set; a missing-disc error names the disc number and the label beside the uuid. `disc list` becomes `status`: the staged bytes, one word for each disc (`packed`, `burned`, `verified C/N`, `verified`, `on disc only`), and one `next:` line that names the action to take. `rebuild-cache` becomes `recover`: it rebuilds the state, not a cache. Defaults close the last flags: the config key `pack.capacity`, a label that defaults to the newest ref name and the disc number, and `NOAHSARK_REPO`, which already existed. `commit`, `pack`, `disc burned 0` and `verify <MOUNT>` now run with no flag. `--json` keeps every exact number the one-word state leaves out. No on-disc byte changed. |
| 0.4.22 | **One restore engine and one report (GitHub issues #32 and #48).** `internal/restore` had two walks over the same snapshot. The single-disc walk is gone: `Restore` and `RestoreWithProgress` now call `RestoreMulti` with one disc root, thus `restoreRootEntry`, `restoreDirContents`, `restoreEntry`, `restoreFile`, `resolveIncludes`, `resolveIncludesDir` and `sumRegularSizes` are deleted from `restore.go`. Every safety rule stays where it was, in `safety.go`. The two engines differed in three places, and the safer behaviour won each one: a missing object no longer stops the walk but names the disc to insert; a snapshot object is also looked for in a run's `catalog/snapobj` copy; and a single-disc restore no longer pre-sums a byte total, so its progress line reports throughput without a percentage. The multi-disc engine gained what only the single-disc engine had: an object that a provided disc holds but cannot verify is reported as damage, naming the file, not as a missing disc. The six report channels (`WithUnsupportedEntry`, `WithOverwriteBlocked`, `WithMetadataFailure`, their record types, five `Manifest` accessors and seven `*Line` functions) are replaced by one `Report` that a restore returns: `[]Problem{Path, Kind, Err}`, a bounded 20 problems held with a count of the rest, one print loop and one summary line. Every skipped path is now named; before, `restore` printed a count alone. An unsupported entry reads `FIFO`, `socket` or `character device`, never `entry type 6`. An object that does not verify fails its own file and the walk continues, in place of stopping the whole restore; the exit code is 1 either way. The exit rule is unchanged: an unsupported entry alone exits 0, and every other kind exits 1. OPERATIONS.md's "Failure policy" gives the kinds in one table, and its restore exit codes lose the stale code 3 row. An empty `SNAPSHOT` argument was quoted back as `ref ""`, which names nothing; `restore`, `ls`, `log` and `plan` now report `no snapshot given` and exit 2. `restore` also names a `DISC-ROOT` that is not a directory, and says which of `OUT-DIR` and the disc root is missing from a two-argument form. No on-disc byte changed. |
| 0.4.21 | **Exit codes 0, 1, 2 everywhere; no command keeps a special code (GitHub issue #35).** The registry is now three codes only: 0 success, 1 a failure at run time (a missing disc, an incomplete cache, a held repository lock, a partial success that still needs the operator's attention), 2 a usage error (a bad option, a bad argument, a bad config value, an unknown command). Code 3, "a required disc or object is missing", and `gc`'s own code 2 for a run-time failure, are both gone; every place that returned 3 now returns 1. `gc` no longer exits 1 for "nothing was eligible": that outcome is success, exit 0, the same as `--dry-run` already gave it; only a staged file `gc` could not unlink is a failure now, exit 1, naming the path and the underlying error, in place of the same silent `deleted 0 object(s)` a real failure and a quiet repository both used to print. `plan` treats an empty cache and an incomplete cache as the run-time conditions they are, exit 1, and keeps exit 2 for an argument that names no known snapshot id or ref. `pack` exits 0 when objects stay STAGED for the next disc: the disc was packed correctly, and the `remaining staged:` line says how much is left, so a script no longer has to treat a normal multi-disc pack as a failure. A new table-driven test, `TestUsageErrorsExitTwo`, runs a usage-error form of every command and checks it exits 2. No on-disc byte changed. |
| 0.4.20 | **One non-blocking exclusive repository lock; no lock for a read-only command (GitHub issue #34).** The shared lock mode, the config key `repo.lock_timeout` and the pid text `AcquireExclusive` wrote into `<repo>/lock` are gone. `internal/repolock.Acquire` takes a non-blocking exclusive lock only, and fails at once, never waiting, when another command already holds it. `plan`, `ls`, `log` and `disc list` take no lock at all; `init`, `commit`, `pack`, `gc`, `disc burned` (and its `--undo` form), `rebuild-cache`, `verify` with a repository and the disc-swap mode of `restore` still take the exclusive lock, since each of them writes the state log, the staging store, a ledger or the config, and `restore`'s disc-swap mode also writes a resumable spool under `<repo>/staging/restore/`. A held lock is a failure at run time: exit code 1, not 2, with one message naming the lock file and saying that another noahsark command runs on the repository. `internal/stage.Open` still cuts a torn tail off `state.db` and reports it, but only the exclusive lock holder ever calls it now; a new `stage.OpenReadOnly`, used by `disc list`, replays the same torn-tail rule without writing to the file, so a `disc list` that races a concurrent append can never truncate bytes the writer has not finished appending. No on-disc byte changed. |
| 0.4.19 | **Go readers stop rejecting a nonzero reserved byte.** FORMAT.md's binary format rules already said a writer zeroes every reserved field and padding byte and a reader does not interpret or reject one; the Go decoders in `internal/format` did not follow that rule, so an old reader refused any disc a later writer used a reserved field on. `Decode` in `run.go`, `disc.go`, `index.go`, `snapshot.go`, `tree.go`, `refs.go`, `discs.go`, `checksum.go`, `blob.go` and `tlv.go` now reads a reserved field or padding byte without checking it, and `ErrReserved` and its `checkZero` helper are gone. A magic, a version, a length, a CRC, a registry value the reader must know, the TLV critical bit and canonical ordering are still checked; those are reader duties FORMAT.md states, not reserved fields. Every writer still zeroes reserved fields, and the golden tests still prove it; a new decode test per structure sets a golden file's reserved byte or padding byte to a nonzero value and confirms `Decode` accepts it and returns the same fields. No on-disc byte changed. |
| 0.4.18 | **Five staging states, one state file, a smaller `gc` (GitHub issue #31).** The staging state record gains `clean_sec`, the unix time of the first successful verify; every later record carries it forward, thus the retention still counts from the first verify and a second verify does not restart it. The companion file `clean_times.db` is deleted. The record is 79 bytes. OPERATIONS.md's "State log record" table now gives that record; the 96-byte design record it gave before was never written by any build. GC-ELIGIBLE and DELETED are replaced by one last state, ON-DISC: a disc holds the object and staging holds no file for it. `gc` writes the ON-DISC record, flushes it, and only then unlinks the staged file; a crash between the two leaves an orphan staged file, and the next `gc` run frees it. `rebuild-cache` records the objects of a fed disc as ON-DISC, and leaves an object the log already knows at its own state; such a disc needs no `disc burned` and no `verify`, and `disc list` prints `on disc only` for it. The guide no longer tells the operator to mark and verify every disc again after a recovery. The state log has one torn-tail rule, applied at open: a partial record at the end, or a last record with a bad CRC, is cut and reported as one warning; a bad record with good records after it is an error that changes no byte. `gc` loses `--yes` and `--keep-snapshots`; a script confirms `--force-after` with a pipe. `gc` never trims the local cache, and the config key `cache.snapshot_depth` is deleted with `internal/cache/trim.go`. The state record and the state log are local files, not disc structures. No on-disc byte changed. |
| 0.4.17 | **A disc and its run are keyed by the disc uuid; a sequence number is a label (GitHub issue #29).** `verify`, `disc burned` and `disc burned --undo` select the state records of a disc by its uuid alone, never by the pair of the uuid and `run_seq`. One disc holds one run, thus the disc uuid identifies the run too. `rebuild-cache` no longer refuses two discs that share a sequence number, and no longer warns which numbers the next `pack` assigns; it still reports a partial rebuild. The merged disc ledger is ordered by the creation time, then by the uuid bytes. A `DISC` argument that is a decimal number and matches two discs is refused as ambiguous, and the refusal lists the uuids; the operator gives a uuid or a uuid prefix. `disc list` no longer prints the `runs` count. `run_seq` and `disc_seq` stay on the disc, stay in the staging state record, and stay in the printed output as a label for the human. The order rule for refs still takes the highest `run_seq`. No on-disc byte changed. |
| 0.4.16 | **The local cache keys a disc by its uuid (GitHub issue #29).** The host assigns `run_seq` from local state, thus after a lost repository two discs can carry the same number. The cache no longer uses that number as a key. The cached index and catalog of a disc move from `runs/<seq>/` to `discs/<disc-uuid>/`, and each cached disc resolves an INDEX Prereqs row through its own DISCS table, never through another disc's table. An object location now carries the disc uuid in place of `run_seq`. The restore plan groups an object it cannot place by disc uuid, or reports `disc unknown`; the JSON plan's `missing_discs` entry carries `disc_uuid` in place of `run_seq`, and a `discs[]` entry no longer carries `runs`. `gc` confirms a staged object in the cached INDEX of its own disc, found by the disc uuid of the state record, and its dry run groups per disc. An incomplete-cache message names the disc and its label, or says that no cached INDEX names the disc. The old `runs/<seq>/` cache layout is ignored, not migrated; `pack` or `rebuild-cache` writes the new layout. The staging state record keeps `run_seq` and still writes it. No on-disc byte changed. |
| 0.4.15 | **`gc` waits for two verified copies (GitHub issue #28).** The staging state record gains a verify count. A successful `verify` of a BURNED object moves it to CLEAN with count 1; a successful `verify` of an object that is already CLEAN keeps it CLEAN and adds 1, and the count stops at 255. The GC-ELIGIBLE and DELETED records carry the count forward. The clean time is the time of the first verify, thus `staging.retain_after_clean` counts from the first verify and a second verify does not restart it. A failed `verify` leaves a CLEAN object alone. The new config key `gc.min_verified_copies`, default 2, is the number of successful verifies an object needs before `gc` may delete it; a value below 1 is a config error. `gc` holds every object below that count, in the dry run and in the real run, and prints one line for each disc it holds objects back for; `--force-after` shortens the retention period only and never passes by the count. `verify` prints the count of the run, and `disc list` prints `verified=C/N` and `verified_copies`. Two identical copies carry the same disc uuid, thus the count counts successful verify passes, not distinct physical discs; the guide says so and gives `gc.min_verified_copies = 1` as the key for an operator with one copy. OPERATIONS.md's "Transition rules", "GC rules", "Verify procedure", `verify`, `gc` and `disc` entries and configuration reference state the rules; the GC rules no longer carry the known gap that named issue #28. The state record is a local file, not a disc structure. No on-disc byte changed. |
| 0.4.14 | **Dead code deleted and the documents aligned to match.** `internal/format/carve.go`, `burn_times.db` and `fed_discs.db` are gone; a disc counts as fed when the state log holds an on-disc record naming its disc uuid. `pack --media`, `verify --image` (`verify` now takes `DISC-ROOT` as its one required positional argument), `rebuild-cache --from-disc`, `rebuild-cache --level`, `rebuild-cache --snapshot`, `gc --verbose` and `gc --progress` are gone; every mention of them is removed from OPERATIONS.md's CLI reference and prose, in place of the removed flags. `commit` no longer byte-compares an existing staged object; it checks the size and writes the object again when the size differs. CLAUDE.md's topic table is corrected to the renamed OPERATIONS.md headings ("11. Burning", "12. Disc lifecycle and closing", "13. Verify and heal"), drops the "Recovery by carving" row and "mirror mode", drops the whole "Phase discipline" section and the phase-tag step, since this build has no phase gate left to check. `docs/guide.md`'s citations of the renamed headings are corrected. No on-disc byte changed. |
...
| 0.4.9 | **Exit code registry aligned with the code.** OPERATIONS.md's exit code registry, CLI reference and "Repository discovery" now state the build's real convention directly, in place of a separate "design" line and an "in the current build" line: 0 success, 1 a failure at run time, 2 a usage error, 3 a required disc or object missing, 4 a design-only precondition failure. `gc` keeps its one deviation, code 2 for a run-time failure and code 1 for "nothing was eligible", now named as the exception it is. "Repository discovery" states which commands treat a missing repository as a usage error (code 2) and which treat it as a run-time read failure through the local cache (code 1). `disc burned`'s exit code for an unresolved `DISC` argument is stated as 2, matching the code, not the earlier 3. Commands not yet built (`burn`, `scrub`, `health`, `sync`, `close`, and the Backlog and Phase 3 commands) keep their design exit codes, renumbered onto this same convention. No byte on a disc changes, and no Go code changes. |
| 0.4.8 | **CLI reference aligned with the Phase 1 build.** OPERATIONS.md's "CLI reference" now gives, for each command that the build has, the syntax, the options, the output and the exit codes of that build: `init --source` and no capacity option; `commit [SOURCE]`; `pack` with a mandatory `--capacity`, `--physical-capacity`, `--out`, `--fec` and `--no-fec`; `verify` with a `DISC-ROOT` and `--repo`; the two `restore` modes (all discs at once, and `--mount` disc swap) with `--disc`, `--discs-dir` and `--plan`; `plan --include`; `rebuild-cache --disc` and `--discs-dir`; `gc --verbose`, `--keep-snapshots` and `--yes`; `disc burned` with `--undo` and the `DISC` resolver; `image build TREE-DIR` with `--capacity` and `--force`; and the progress options. Each Phase 1 option that the build does not have yet stays in its option table, and a note below the table names it. No later-phase or Backlog item moved. "Transition rules" gains the `disc burned` rule. The configuration reference gives `fec.scheme` the default `none` and says that `staging.dir` is relative to the repository directory. No byte on a disc changes. |
| 0.4.7 | **Local cache implemented.** `internal/cache` now keeps, under OPERATIONS.md's "Local cache layout": each run's INDEX, REFS and DISCS under `runs/<seq>/`, every cached snapshot object under `snapshots/<id>`, every tree object reachable from a cached snapshot under `trees/<id>`, and a per-snapshot completeness record in `state.txt`. `pack` writes the cache after a successful run, and `rebuild-cache` writes it from every disc it is given, both through one `cache.WriteFromRoot` path, since a freshly packed run tree and a mounted disc root share one layout. A cache read that finds a snapshot's tree set incomplete reports an `*cache.IncompleteError` naming the missing tree and, where a cached run's INDEX Prereqs table or DISCS table can resolve it, the run or disc that holds it. A new config key, `cache.snapshot_depth`, is parsed and stored for a later gc to use; this build enforces no limit. No on-disc byte or structure changed. |
| 0.4.6 | **DVD entries added to the media type registry.** The media type registry gains id 5, `DVD+R SL 4.7 GB`, and id 6, `DVD-R SL 4.7 GB`, both Phase 1; the reserved range moves to 7-255. `media_type` stays informational, so this changes no rule and no reader behaviour. `pack` no longer refuses a `dvd+r` or `dvd-r` `--capacity` preset or `--media` value; it now maps each to its own media type id, matching every BD preset. |
| 0.4.5 | **Pack carries every known ref forward.** Fixed a defect where `pack` wrote into a run's `refs.bin` only the ref or refs named on its own command line, so a disc other than the one that packed a given ref carried no record for it, contradicting FORMAT.md's rule that REFS is replicated in full on every run. `pack` now merges the refs already known to the repository, each keeping the `run_seq` of the run that packed it, with the refs named on this command line, each stamped with this run's `run_seq`, and writes the union into `refs.bin`; a local refs ledger, `refslog.bin`, persists the merged set for the next pack to extend, and `rebuild-cache` restores it from the discs it reads. Restore-side, `internal/restore.Source.Refs` now reads REFS from every provided disc and merges by name, the higher `run_seq` winning a conflict, instead of trusting the first disc's copy alone; this also fixes `restore --discs-dir` failing to resolve a ref when disc directories sort with an unrelated disc first. OPERATIONS.md's refs section corrected to match: rule 2 no longer limits `pack` to only the local records with `run_seq` 0. No on-disc byte or structure changed. |
| 0.4.4 | **FEC made optional, off by default.** The FEC scheme registry gains id 0, `none`, now the default; id 1, `rs255-gf8`, is unchanged and stays available. A run with `fec_scheme` 0 writes no `checksum.bin` and no parity files, and its INDEX carries no role 10 or role 11 rows; `RUN.bin` and `RUN2.bin` still exist. The stream definition still fixes the file order for a scheme 0 run. A reader verifies a scheme 0 run by content id and file hash alone; `heal` refuses a scheme 0 run and reports that it has no FEC. The decision: burning two identical discs is the primary redundancy this project relies on, and Reed-Solomon parity on one disc is a reserve feature a user opts into, not a default cost on every disc. `FORMAT.txt` (Appendix A) is not regenerated by this change, so a scheme 1 run's on-disc bytes, including its embedded `FORMAT.txt` copy, stay identical to document version 0.4.3. |
