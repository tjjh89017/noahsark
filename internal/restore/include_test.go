...
func TestRestoreIncludeSingleFile(t *testing.T) {
	...
	if err := Restore(treeDir, snapID, outDir, WithInclude([]string{inc})); err != nil {
		t.Fatal(err)
	}
...
func TestRestoreIncludeTwoIncludesUnion(t *testing.T) {
	...
	if err := Restore(treeDir, snapID, outDir, WithInclude(incs)); err != nil {
		t.Fatal(err)
	}
...
func TestRestoreIncludeUnmatchedFailsBeforeWriting(t *testing.T) {
	...
	err := Restore(treeDir, snapID, outDir, WithInclude([]string{badInclude}))
	if err == nil {
