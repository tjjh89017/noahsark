...
func assertPeakUnderBudget(t *testing.T, label string, peak uint64) {
	t.Helper()
	t.Logf("%s peak heap+stack: %d bytes (%.1f MiB)", label, peak, float64(peak)/(1<<20))
	if peak > memPeakBudget {
		t.Fatalf("%s peaked at %d bytes, want under %d (%.1f MiB budget)", label, peak, memPeakBudget, float64(memPeakBudget)/(1<<20))
	}
}
...
// stageMemoryFixture stages a source tree holding at least
// memFixtureBytes of pseudo-random content, streamed to disk, and
// returns the staging directory and the snapshot id.
func stageMemoryFixture(t *testing.T) (string, object.ID) {
	t.Helper()
	srcDir := t.TempDir()
...
func TestBuildMemoryBounded(t *testing.T) {
	if testing.Short() {
		t.Skip("memory assertion test, skipped under -short")
	}
	stagingDir, snapID := stageMemoryFixture(t)
	outDir := t.TempDir()
	...
	assertPeakUnderBudget(t, "Build", sampler.Stop())
}

func TestReadMemoryBounded(t *testing.T) {
	if testing.Short() {
		t.Skip("memory assertion test, skipped under -short")
	}
	stagingDir, snapID := stageMemoryFixture(t)
	...
	assertPeakUnderBudget(t, "Read", sampler.Stop())
}

func TestPackMemoryBounded(t *testing.T) {
	if testing.Short() {
		t.Skip("memory assertion test, skipped under -short")
	}
	stagingDir, snapID := stageMemoryFixture(t)
	...
	outDir := t.TempDir()
	opts := packOpts(stagingDir, snapID, outDir, memFixtureCapacitySectors, 1, l)

	sampler := startPeakMemSampler()
	if _, err := Pack(opts); err != nil {
		t.Fatal(err)
	}
	assertPeakUnderBudget(t, "Pack", sampler.Stop())
}
